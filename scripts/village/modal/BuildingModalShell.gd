...
func set_body(body: Control) -> void:
	...
	# Prepare and parent
	_body = body
	# do NOT rename (was hardcoded "VendorBody" before)
	_body.top_level = false
	_body.visible = true
	_body.mouse_filter = Control.MOUSE_FILTER_PASS

	host.add_child(_body)  # no defer; host exists now
	...
	# Bind + context + enter (ok before our _ready())
	...
	_dump_host_children()          # debug: show children under BodyHost
	_refresh_footer_actions(_build_body_ctx())  # show actions if provided
...
func _setup_footer() -> void:
	if _btn_close:
		...
		_btn_close.custom_minimum_size = Vector2(_BTN_MIN_W, _BTN_MIN_H)
	if _btn_upgrade:
		_btn_upgrade.text = "Upgrade"
		_btn_upgrade.custom_minimum_size = Vector2(_BTN_MIN_W, _BTN_MIN_H)
	if _primary_actions:
...
func _on_upgrade_button() -> void:
	# If we're already on the Upgrade tab and it's ready, emit now.
	if _upgrade_tab_idx >= 0 and is_instance_valid(_tabs) and _tabs.current_tab == _upgrade_tab_idx and _upgrade_ready:
		...
		_emit_upgrade()
		return
	# Otherwise switch to the Upgrades tab
...
func _init_tabs_from_body() -> void:
	...
	# Shell-owned Upgrades tab (always last)
...
func _on_tab_changed(idx: int) -> void:
	...
	_show_upgrade_panel(false)
	# Forward to body with same index (since Upgrades is appended at end)
	if _body != null and _body.has_method("on_tab_changed"):
		_body.call("on_tab_changed", idx)

func _show_upgrade_panel(show: bool) -> void:
	# Lazy-create the panel; toggle visibility against the body
	if show:
		if not is_instance_valid(_upgrade_panel):
			_upgrade_panel = _create_upgrade_panel()
			_body_host.get_parent().add_child(_upgrade_panel) # sibling under BodyHost container
...
func _refresh_upgrade_panel() -> void:
	...
	if reqs_box:
		...
		var l := Label.new()
		l.text = "Requires: %d gold, %d shards%s" % [r_gold, r_shards, (" + quest: " + r_qgate) if r_qgate != "" else ""]
	...
	# Reflect state to footer + remember readiness
	...
	# Optional: tweak button label so it’s obvious
...
func refresh_all() -> void:
	...
	# Use dynamic resolver (Vendor vs Trainer etc.)
...
func _apply_staffing(s: Dictionary) -> void:
	...
	# Prefer selecting the CURRENT assignment; fall back to first entry
	...
	# Now compute the button states based on current vs selected
	_refresh_staff_buttons()

	# Left panel portrait/xp
	if staffed:
...
func _refresh_footer_actions(ctx: Dictionary) -> void:
	...
	for a_any in actions:
		...
		var b := Button.new()
		b.text = label
		b.custom_minimum_size = Vector2(_BTN_MIN_W, _BTN_MIN_H)
		b.disabled = not enabled
		b.pressed.connect(_on_primary_action.bind(StringName(id)))
		# Insert before the Upgrade button, keep order left→right
...
func _debug_fix_layout_flags() -> void:
	...
	if tabs_ctrl:
		...
		tabs_ctrl.custom_minimum_size.y = 32.0
...
func _setup_staffing_row() -> void:
	if is_instance_valid(_roster):
		_roster.custom_minimum_size = Vector2(_ROSTER_MIN_W, _STAFF_BTN_MIN_H)
		# (re)wire once
	...
	if is_instance_valid(_btn_assign):
		...
		_btn_assign.custom_minimum_size = Vector2(_STAFF_BTN_MIN_W, _STAFF_BTN_MIN_H)

	if is_instance_valid(_btn_unassign):
		if _btn_unassign.text.strip_edges() == "":
			_btn_unassign.text = "Unassign"
		_btn_unassign.custom_minimum_size = Vector2(_STAFF_BTN_MIN_W, _STAFF_BTN_MIN_H)
...
func _refresh_staff_buttons() -> void:
	...
	# Enable Assign when a selection exists AND it's different from current
	if is_instance_valid(_btn_assign):
		_btn_assign.disabled = (not has_sel) or is_current

	# Enable Unassign only when selection IS the current (and there is a current)
	if is_instance_valid(_btn_unassign):
