...
signal closed  # emit when closing so the service can suppress reliably
...
func _cache_ui() -> void:
	...
	var _panel := root_v.get_parent() as Panel
	if _panel != null:
		_panel.set_anchors_preset(Control.PRESET_FULL_RECT)
		_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		_panel.size_flags_vertical = Control.SIZE_EXPAND_FILL

	var _margin := _panel.get_node_or_null(^"Margin") as MarginContainer if _panel != null else null
	if _margin != null:
	...
	# --- size flags for tabs/pages/scrolls/grids (critical for visibility) --------------
...
func _on_close_pressed() -> void:
	print("[WildModal] close pressed -> emitting 'closed'")
	emit_signal("closed")
	var modal := _find_modal_root()
...
func _find_modal_root() -> Node:
	var n: Node = self
	while n != null:
		if n.name.findn("Modal") != -1 or n.has_method("close"):
			return n
		n = n.get_parent()
	return null
