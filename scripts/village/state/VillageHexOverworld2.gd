...
func _ready() -> void:
	...
	# ── NEW: prefer authoritative VillageService snapshot, fall back to provider ──
	...
	if debug_logging:
		print("[VHexOverworld2] snapshot ready seed=", snapshot.get("seed", seed), " radius=", radius, " tiles=", tile_count)
