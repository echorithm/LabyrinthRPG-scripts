...
func save_snapshot(snap: Dictionary) -> bool:
	var paths := VillageMapPaths.new()
	var schema := VillageMapSchema.new()
	var builder := VillageMapSnapshotBuilder.new()
	var resolver := _make_resolver()
	if resolver == null:
		push_error("[VillageService] save_snapshot: BaseTileCatalog missing; abort")
		return false
	var provider := VillageMapProvider.new(paths, schema, builder, resolver)
...
func sync_with_active_vmap() -> void:
	...
	if not FileAccess.file_exists(act_path):
		...
		return
	var f := FileAccess.open(act_path, FileAccess.READ)
	if f == null:
		return
...
func _bootstrap_or_load() -> void:
	...
	if debug_logging:
		print("[VillageService] start | seed=", _seed, " radius=", ring_radius, " fresh=", fresh)
	...
	if debug_logging:
		...
		print("[VillageService] ready | tiles=", tile_count, " seed=", _seed)
