...
static func _xp_to_next(level: int) -> int:
	...
	return int(XpTuning.xp_to_next(level))

# --- public API --------------------------------------------------------------

## Ensure a role track exists and is a Dictionary (not an int). Safe for legacy saves.
static func ensure_role_track(npc_id: StringName, role: String, slot: int = SaveManager.DEFAULT_SLOT) -> void:
	...
	for i in npcs.size():
		...
		if track_any == null:
			rl[role] = {
				"level": 1,
				"previous_xp": 0,
				"time_assigned": null, # set by anchor on assign
				"xp_current": 0,
				"xp_to_next": 90,
			}
			n["role_levels"] = rl
	...
	if changed:
		snap["npcs"] = npcs
		SaveManager.save_village(snap, slot) # (snapshot, slot) ← correct order
...
static func on_reassign(role: StringName, prev_staff_id: StringName, new_staff_id: StringName, now_min: float, slot: int = SaveManager.DEFAULT_SLOT) -> void:
...
static func settle_all_assigned(now_min: float, slot: int = SaveManager.DEFAULT_SLOT) -> void:
...
static func settle_for_instance(instance_id: StringName, now_min: float, slot: int = SaveManager.DEFAULT_SLOT) -> Dictionary:
	var v: Dictionary = SaveManager.load_village(slot)
	var b_any: Variant = v.get("buildings", [])
	var buildings: Array = (b_any as Array) if (b_any is Array) else []
...
static func settle_role(npc_id: StringName, role: StringName, now_min: float, slot: int, freeze_after: bool) -> void:
	...
	for i in npcs.size():
		...
		if levels > 0:
			...
			tr["xp_current"] = accum
			tr["time_assigned"] = now_min  # reset anchor ONLY on level-up (your policy)
			rl[key] = tr
			n["role_levels"] = rl
			npcs[i] = n
			changed = true
	...
	if changed:
		v["npcs"] = npcs
		SaveManager.save_village(v, slot) # (snapshot, slot)
		_log("settle_role: saved slot=%d" % slot)

# --- helpers -----------------------------------------------------------------

## Anchor the role at 'now_min' (used on assignment). Does not change previous_xp.
static func anchor_role(npc_id: StringName, role: String, time_now_min: float, slot: int = SaveManager.DEFAULT_SLOT) -> void:
	var snap: Dictionary = SaveManager.load_village(slot)
	var npcs_any: Variant = snap.get("npcs", [])
	var npcs: Array = (npcs_any as Array) if (npcs_any is Array) else []
	var anchored := false
	...
	for i in npcs.size():
		...
		if track_any == null or track_any is int:
			_log("anchor_role: missing/legacy track for npc=%s role=%s -> creating default before anchor" % [String(npc_id), role])
