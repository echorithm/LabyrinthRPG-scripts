	...
	if enable_mouse:
		if event is InputEventMouseButton:
			...
			if e.button_index == MOUSE_BUTTON_LEFT:
				...
				var dir: float = (1.0 if e.button_index == MOUSE_BUTTON_WHEEL_UP else -1.0)
...
func _zoom_at(factor: float, focus_screen: Vector2) -> void:
	...
	var focus: Vector2 = (focus_screen if zoom_to_cursor else (get_viewport_rect().size * 0.5))
