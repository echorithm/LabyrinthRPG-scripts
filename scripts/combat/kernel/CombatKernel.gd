...
func resolve(
	...
	if intent_id.begins_with("IT_heal"):
		var base_power: float = float(ability_row.get("base_power", 0))
		var pow_scalar: float = _power_or_heal_scalar_from_row(ability_row, ability_level)
		var heal_amt: int = max(1, int(round(base_power * pow_scalar)))
		print("[Kernel] HEAL calc aid=%s lvl=%d base=%.2f scalar=%.3f -> heal=%d" %
			[use.ability_id, ability_level, base_power, pow_scalar, heal_amt])
		...
		for d in ally_defs:
			...
			events.append({"type": "heal_applied", "source_id": atk.id, "target_id": d.id, "amount": heal_amt})
