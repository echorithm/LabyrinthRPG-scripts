...
func _build_player_runtime() -> PlayerRuntime:
	...
	# --- Read pools & attributes from RUN (source of truth mid-run)
	...
	# Attributes: prefer RUN.player_attributes; fallback to a sane default if missing.
	var base_stats: Dictionary = {}
	if rs.has("player_attributes") and rs["player_attributes"] is Dictionary:
		base_stats = (rs["player_attributes"] as Dictionary).duplicate()
	else:
		# very first boot fallback; you can remove this once RUN always contains attributes
		base_stats = {
...
func _extract_player_attributes_from_run() -> Dictionary:
	...
	_dbg("[BL] RUN missing 'player_attributes' — using defaults.")
...
func _on_battle_finished(result: Dictionary) -> void:
	...
	# 1) play death anim if needed (still locked)
	...
	# 2) On victory, GRANT + SHOW rewards NOW, while controls are still locked.
	if outcome == "victory":
		# Build minimal context for rewards
		...
		if _current_role == "elite":
			src = "elite"
		elif _current_role == "boss":
			src = "boss"
		else:
			src = "trash"
		var enemy_name := _current_monster_slug

		# This should compute XP, grant via RewardService, show modal, and await until closed.
		# If you kept _roll_and_grant_victory_rewards() and a separate _show_rewards_modal(),
		# replace those two calls with this single await:
		var lr := LootReward.new()
		get_tree().root.add_child(lr)        # put it in the tree so get_node_or_null works
		await lr.encounter_victory(src, floor_i, enemy_name, rng_seed)
		lr.queue_free()

	# 3) NOW restore camera and unlock movement
	_restore_camera_after_battle()
	var player := _find_player({})
	_lock_player(player, false)

	# 4) Persist HP/MP from the battle result into RUN
	...
	if outcome == "defeat":
		...
		get_tree().change_scene_to_file("res://scenes/TestingMenu.tscn")
	else:
		# Unblock RNG loop
		if not _finish_sent:
...
func _roll_and_grant_victory_rewards(enemy_slug: StringName, role_source: String) -> Dictionary:
	...
	# IMPORTANT: Let LootReward do the granting; do NOT grant again here.
	var loot_api := LootReward.new()
	get_tree().root.add_child(loot_api)  # in-tree so its get_node_or_null() works

	var pack: Dictionary = await loot_api.encounter_victory(
		source,        # "trash" | "elite" | "boss"
		floor_i,
		enemy_name,
		run_seed,
		0              # post_boss_shift_left (if you use it)
	)

	loot_api.queue_free()

	# If LootReward returns both loot + receipt, pass those through to the modal.
	# If it only returns loot, make a minimal receipt so the modal shows something.
