...
func _build_player_runtime() -> PlayerRuntime:
	...
	# (Optional but nice for visibility)
	if pr.skill_tracks.size() > 0:
		_dbg("Player skills", {"tracks": pr.skill_tracks.size()})
...
func _on_battle_finished(result: Dictionary) -> void:
	...
	print("BL, monster_level ", _current_monster_level)
	result["monster_display_name"] = _current_monster_display_name
	print("BL, monster_display_name ", _current_monster_display_name)
	result["monster_slug"] = String(_current_monster_slug)
	print("BL, monster_slug ", _current_monster_slug)
	result["role"] = _current_role
	print("BL, role ", _current_role)
	...
	# 2) Rewards only on victory (unchanged) ...
	if outcome == "victory":
		...
		var enemy_name := _current_monster_slug
		_dbg("Victory; rolling rewards", {"source": src, "floor": floor_i, "enemy": String(enemy_name)})

		var skill_xp_entries: Array[Dictionary] = _collect_player_skill_usage_for_rewards()
		_dbg("Skill usage → XP entries", {"entries": skill_xp_entries})

		var lr := LootReward.new()
		get_tree().root.add_child(lr)
		var pack: Dictionary = await lr.encounter_victory(
			src,                          # source/role source ("trash"/"elite"/"boss")
			floor_i,
			String(_current_monster_slug),# enemy_id (slug)
			rng_seed,
			0,
			skill_xp_entries,
			_current_monster_display_name,# NEW: enemy_display_name
			_current_monster_level,       # NEW: enemy_level
			_current_role                 # NEW: role_hint
		)
		...
		var gl := get_node_or_null(^"/root/GameLog")
		if gl != null:
			gl.call("info", "reward", "Rewards granted",
				{"source": src, "floor": floor_i, "enemy": String(enemy_name), "skill_xp_entries": skill_xp_entries.size()})

	# 3) Restore camera and unlock movement (unchanged)
	_restore_camera_after_battle()
	var player := _find_player({})
	_lock_player(player, false)

	# 4) Persist HP/MP from battle into RUN  ✅
	...
	# 🔁 NEW: refresh RunState mirrors so UI/next encounter sees the updated pools
	var rs_node: Node = get_node_or_null(^"/root/RunState")
	if rs_node and rs_node.has_method("reload_and_broadcast"):
		rs_node.call("reload_and_broadcast", SaveManager.DEFAULT_SLOT)
	elif rs_node and rs_node.has_method("reload"):
		# Fallback if older RunState without the helper
		rs_node.call("reload", SaveManager.DEFAULT_SLOT)

	# 5) Cleanup anchor/temp nodes (unchanged)
	_cleanup_after_battle(outcome)

	# 6) Route by outcome (unchanged)
	if outcome == "defeat":
		var DefeatFlow := preload("res://persistence/flows/DefeatFlow.gd")
		DefeatFlow.execute(SaveManager.DEFAULT_SLOT)
		...
		if not _finish_sent:
			...
			if router != null and router.has_method("finish_encounter"):
				_finish_sent = true
				router.call("finish_encounter", result)  # result now includes monster_level
...
static func _apply_passives(stats: Dictionary, mods: Dictionary, caps: Dictionary) -> Dictionary:
	...
	# Flat & % bonuses from mods (affix, village)
...
func _collect_player_skill_usage_for_rewards() -> Array[Dictionary]:
	# Reads per-battle usage from PlayerRuntime if present.
	# Expected API on PlayerRuntime:
	#   func consume_skill_usage() -> Dictionary[String, SkillUsageRow] or Dictionary[String, Dictionary]
	var out: Array[Dictionary] = []

	if _pr == null:
		_dbg_warn("No PlayerRuntime; cannot collect skill usage.")
		return out

	var usage_any: Variant = null
	if _pr.has_method("consume_skill_usage"):
		usage_any = _pr.call("consume_skill_usage")
	else:
		_dbg_warn("PlayerRuntime missing consume_skill_usage(); no skill XP.")
		return out

	if not (usage_any is Dictionary):
		_dbg_warn("consume_skill_usage() did not return a Dictionary; got=%s" % [typeof(usage_any)])
		return out

	var usage: Dictionary = usage_any as Dictionary

	# --- Level-diff factor (player vs the actual spawned monster level) ---
	...
	var m_lvl: int = _current_monster_level
	var dr: float = XpTuning.level_diff_factor(p_lvl, m_lvl)
	...
	for ability_id_any in usage.keys():
		...
		var row_any: Variant = usage[ability_id]
		if row_any is SkillUsageRow:
			var r := row_any as SkillUsageRow
			uses = r.uses
			hits = r.hits
			crits = r.crits
			total_damage = r.total_damage
		elif row_any is Dictionary:
			var d := row_any as Dictionary
			uses = int(d.get("uses", 0))
			hits = int(d.get("hits", 0))
			crits = int(d.get("crits", 0))
			total_damage = int(d.get("total_damage", 0))
		else:
			continue

		# Base per-ability total (integer math); /50 uses integer division via int(...)
		var base_xp: int = XP_PER_USE * uses \
			+ XP_PER_HIT * hits \
			+ XP_PER_CRIT * crits \
			+ XP_PER_50_DMG * int(total_damage / 50)

		# Apply level diff factor and clamp to a sane per-encounter bound.
		# Match AbilityXPService's commit clamp semantics (1..100) so tallies feel consistent.
		var scaled: int = clampi(int(round(float(base_xp) * dr)), 1, 100)

		if scaled > 0:
			out.append({"id": ability_id, "xp": scaled})

	_dbg("Skill usage tallied", {
		"player_level": p_lvl,
		"monster_level": m_lvl,
		"dr": dr,
		"count": out.size(),
