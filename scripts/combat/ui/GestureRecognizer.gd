...
static func _builtin_specs() -> Dictionary:
	return {
		"slash": { "dir":"h","horiz_slope_max":0.40,"straight_max":0.08,"detour_max":1.04,"corners_max":0 },
...
static func _gate_verbose(spec: Dictionary, f: Dictionary, failed: PackedStringArray) -> bool:
	...
	if spec.has("corners_max") and int(f["corners60"]) > int(spec["corners_max"]):
		failed.append("corners_max"); return false
	if spec.has("corners_min") and not has_strong_shape_gate and int(f["corners60"]) < int(spec["corners_min"]):
		failed.append("corners_min"); return false
	if spec.has("corners_range"):
		var r: Vector2i = spec["corners_range"]
		var k: int = int(f["corners60"])
		if k < r.x or k > r.y:
