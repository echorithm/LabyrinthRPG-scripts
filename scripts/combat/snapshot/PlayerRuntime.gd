...
func to_actor_snapshot() -> Dictionary:
	...
	if typeof(ability_levels) == TYPE_DICTIONARY:
		...
		for i in akeys.size():
			...
			# If we have an explicit unlock map, enforce it.
	...
	return {
		...
		# ✅ Only unlocked abilities are exposed to the kernel/UI now.
		...
		# ✅ Make both helpers.skills_unlocked and the raw tracks available to consumers.
