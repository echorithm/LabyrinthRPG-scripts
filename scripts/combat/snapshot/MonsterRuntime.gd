...
static func from_alloc(mc: Dictionary, alloc: Dictionary, role_s: String = "trash") -> MonsterRuntime:
	...
	# Allocation
	rt.level_baseline = int(alloc.get("level_baseline", int(mc.get("level_baseline", 1))))
	rt.final_level = int(alloc.get("final_level", rt.level_baseline))
	...
	var built := rb.build_for_monster(stats_block)
	...
	#  - otherwise derive per row's `skill_level_baseline` + optional allocator bonus (e.g. “+ability lv=12”)
	var levels_from_alloc: Dictionary = (alloc.get("ability_levels", {}) as Dictionary)
	var ability_level_bonus: int = int(alloc.get("ability_level_bonus", 0))  # safe optional
