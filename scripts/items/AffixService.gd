...
static func roll_affixes(ilvl: int, rarity_code: String, slot_name: String, base_item_id: String, seed_tuple: Array) -> Array[Dictionary]:
	var reg := Registry.new()
	reg.ensure_loaded()

	var count: int = reg.rarity_affix_count(rarity_code)
	if count <= 0:
		return []
	...
	# Build weighted list filtered by rarity gates
	...
	var rmult: float = reg.rarity_multiplier(rarity_code)
	...
	for i in range(count):
		...
		if pick_id.is_empty():
			continue
		var def: Dictionary = _S.to_dict(defs.get(pick_id, {}))

		# Draw quality in band
		var q: float = qmin + (qmax - qmin) * rng.randf()
		var value_units: Dictionary = _evaluate_magnitude(def, ilvl, rmult, q)

		var row: Dictionary = {
			"id": pick_id,
			"effect_type": String(_S.dget(def, "effect_type", "")),
		...
		if units_str != "":
			row["units"] = units_str
		var params: Dictionary = _S.to_dict(def.get("params", {}))
