...
# Percent power bonus per level (weapons use 'power_pct'; heals may use 'heal_pct')
...
# Multiplicative scalar to apply to base power/heal given an ability level (>=1).
...
func power_scalar_for_level(ability_id: StringName, level: int) -> float:
	...
	# level 1 gets 0% bonus; level N gets (N-1)*per
	return 1.0 + (float(lvl - 1) * per) * 0.01

# Number of milestones reached at this level (every 5 levels).
func milestones_reached(level: int) -> int:
	return max(0, int(floor(float(max(1, level)) / 5.0)))

# Rider snapshot for a given level (applies % tuners and duration steps).
...
# 10-point stat grant for ONE milestone based on stat_bias weights (sum=10).
# Use this per milestone at award-time. If you want totals up to a level,
# multiply by milestones_reached(level).
func milestone_stat_grant(ability_id: StringName) -> Dictionary:
	...
	# Build a safe, fixed-order list to avoid drift
	...
	if sum_w <= 0.0:
		# default: nothing, or put all 10 into END for safety
		grants["END"] = 10
		return grants

	# Proportional split with largest remainder (stable & deterministic)
	var target: int = 10
	...
	for i in range(keys.size()):
		idxs.append(i)
	# sort by remainder desc, then by key name asc for determinism
