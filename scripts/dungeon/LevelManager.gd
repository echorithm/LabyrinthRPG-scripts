...
func _apply_state(state: Dictionary, backtracking: bool) -> void:
	...
	TimeService.begin_floor(_base_width, _base_height, w_now, h_now, rooms_inc_every, _current_floor, 0.0, SaveManager.DEFAULT_SLOT)
...
func _post_ready_sync() -> void:
	...
	_ensure_run_ready(SaveManager.DEFAULT_SLOT)
...
func _ensure_run_ready(slot: int = SaveManager.DEFAULT_SLOT) -> void:
	# If run.json is missing, create it from META and sync RunState cache immediately.
	if not SaveManager.run_exists(slot):
		# Canonical mirror (creates fresh run.json with full pools)
		if SaveManager.has_method("start_or_refresh_run_from_meta"):
			SaveManager.start_or_refresh_run_from_meta(slot)
		else:
			# Fallback (shouldn't happen in your tree)
			SaveManager.load_run(slot)  # forces defaults/mirror path

		# Sync the autoload cache now so no stale autosave can overwrite fresh pools
		if Engine.has_singleton("RunState") or has_node(^"/root/RunState"):
