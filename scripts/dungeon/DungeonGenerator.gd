...
@export var treasure_anchor_y_offset_m: float = 0.0
...
func _generate_and_paint() -> void:
	...
	if removed_fc > 0:
		print("[COL CLEAN] removed floor tiles under prefabs =", removed_fc)
...
func _dbg_runtime_prefabs(root: Node) -> void:
	...
	if rt == null:
		print("[PFDBG] RuntimePrefabs: <none>")
		return
	print("[PFDBG] RuntimePrefabs children=", rt.get_child_count())
...
func build_anchors_for_floor(floor: int) -> void:
	...
	for i in range(elites.size()):
		...
		print("[DG][Anchor] Elite_%03d cell=(%d,%d) world=(%.2f, %.2f, %.2f)"
			% [i, c.x, c.y, w.x, w.y, w.z])
	...
	for i in range(treasures.size()):
		...
		print("[DG][Anchor] Treasure_%03d cell=(%d,%d) world=(%.2f, %.2f, %.2f)"
			% [i, c2.x, c2.y, w2.x, w2.y, w2.z])

	var reg: Node = get_node_or_null("/root/AnchorRegistry") as Node
	if reg != null:
		reg.call("set_floor_anchors", floor, elite_xforms, treasure_xforms)

	print("[DG] Anchors built for floor %d  elites=%d treasures=%d  candidates=%d"
		% [floor, elites.size(), treasures.size(), candidates.size()])
...
func populate_elites_and_boss(floor: int) -> void:
	# Clear previous population if any
	...
	if elite_root != null:
		...
		for n in elite_root.get_children():
			...
			if reserved_hit:
				print("[DG][Populate] SKIP Elite_%03d — anchor on reserved cell=%s" % [i, str(cell)])
				i += 1
				continue
			...
			print("[DG][Populate] Elite_%03d id=%s at world=%s" % [i, String(pick), str(pos)])
			i += 1
	...
	if floor > 0 and (floor % 3) == 0:
		...
		if boss_anchor != null:
			...
			if reserved_b:
				print("[DG][Populate] SKIP Boss — anchor on reserved cell=%s" % [str(cell_b)])
				return
			...
			add_child(b) # (keep consistent with earlier behavior)
			print("[DG][Populate] Boss id=%s at world=%s" % [String(boss_id), str(pos_b)])
	...
	if treasure_root != null:
		...
		for n2: Node in treasure_root.get_children():
			...
			print("[DG][Populate] Treasure_%03d at world=%s" % [ti, str(pos2)])
			ti += 1
