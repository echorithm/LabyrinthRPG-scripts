...
func _set_torch_enabled(torch_root: Node, enable: bool) -> void:
	...
	# If enabling and none exist, create one so prefab can’t be dark
	if enable and omnis.is_empty():
		var nl: OmniLight3D = OmniLight3D.new()
		nl.name = "Omni_Auto"
		if torch_root is Node3D:
			(torch_root as Node3D).add_child(nl)
			omnis.append(nl)
			_dbg("[Torch] created fallback Omni on %s" % str((torch_root as Node3D).get_path()))
