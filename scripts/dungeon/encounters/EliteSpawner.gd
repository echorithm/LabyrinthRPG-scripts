...
@export var monster_id: StringName = &"flying_demon"
...
@export var debug_rotation: bool = false         # no rotation prints now
@export var debug_disable_rotation: bool = false # hard-kill both idle + big yaw (to isolate jitter)
@export var disable_idle_root_tracks: bool = true

# Use AnimAuto helper to pick & loop Idle (preferred path)
@export var use_anim_auto: bool = true

# --- Mimic (Chest) support ---
# When true, pre-battle behavior is prop-like (no idle sweep/rotation); tries to play IdleChest.
@export var mimic_mode: bool = false
# Slugs that should auto-enable mimic_mode on _ready (editor override still wins).
@export var mimic_slugs: PackedStringArray = PackedStringArray(["chest_monster"])
# Clip name to try when mimic_mode is true.
@export var mimic_idle_name: StringName = &"IdleChest"

# Detection
@export var vision_range_m: float = 7.0
@export var vision_fov_deg: float = 35.0          # half-angle
@export var proximity_range_m: float = 3.0
@export var require_line_of_sight: bool = true
@export var cam_eye_height: float = 1.6
@export var vision_query_mask: int = 0xFFFFFFFF   # LoS mask
...
# Idle animation (fallback when not using AnimAuto)
...
# --- Fixpack-0: flee grace (steps)
@export var flee_cooldown_steps: int = 6
...
func _ready() -> void:
	...
	if _router == null:
		push_error("[EliteSpawner] EncounterRouter not found.")
	else:
	...
	# Initialize yaw from current transform (after pivot exists)
	_yaw_accum_rad = _yaw_from_basis((_yaw_pivot.global_transform.basis if _yaw_pivot != null else global_transform.basis))

	# --- Auto-enable mimic if slug is in list (editor override wins if already true/false) ---
	if not mimic_mode:
		var slug_s: String = String(monster_id)
		for s in mimic_slugs:
			if slug_s == String(s):
				mimic_mode = true
				break
	# Apply mimic overrides before any animation decisions.
	if mimic_mode:
		_apply_mimic_overrides()

	# Try AnimAuto first (only when not in mimic mode; mimic handles idle manually)
	if use_anim_auto and not mimic_mode:
		var root: Node = (_spawned_visual if _spawned_visual != null else self)
		_anim_auto_owns_idle = AnimAuto.play_idle(root)

	# Fallback to direct AP control
	if not _anim_auto_owns_idle:
		_ap = _find_anim_player_in(_spawned_visual if _spawned_visual != null else self)
		if mimic_mode:
			_idle_name = mimic_idle_name
		else:
			_resolve_idle_name()
		_try_play_idle_now()
		if disable_idle_root_tracks and not mimic_mode:
	...
	if debug_logs:
		print("[EliteSpawner] READY id=", monster_id, " power=", power_level,
			" path=", get_path(), " has_mesh=", vis != null, " mimic_mode=", mimic_mode)
	_my_path_str = String(get_path())

	set_physics_process(true)

func _apply_mimic_overrides() -> void:
	# Disable all pre-battle movement/scan behavior.
	rotate_enabled = false
	reorient_enabled = false
	# Ensure AnimAuto won't steal control; we will drive a specific idle clip.
	_anim_auto_owns_idle = false

func _process(_dt: float) -> void:
	if not _armed or _player_in_range == null:
		return
	# World gate (ADR-001) and flee cooldown also block interaction-triggered starts
	if _is_world_gated() or _is_under_cooldown_here():
		return
...
func _physics_process(dt: float) -> void:
	...
	# World gate (ADR-001): never start if Director is busy (encounter/rewards) or paused
	if _is_world_gated():
		_ensure_idle_playing()
		return

	# Re-aggro grace after a flee (step-indexed via AnchorRegistry)
	if _is_under_cooldown_here():
		_ensure_idle_playing()
		_update_idle_rotation(dt, null)
		_update_big_reorient(dt, null)
		return
	...
	if dist <= proximity_range_m:
		if debug_logs:
			print("[EliteSpawner] proximity trigger (", dist, " m) → player CTB +", player_start_bonus_pct, "%")
		_pause_any_non_idle()
		_start_battle(player, player_start_bonus_pct, 0)
		return

	# 2) Vision cone with hysteresis + LoS
	...
	if not inside_cone and _alerting:
		...
		if inside_cone:
			...
			if _alert_t >= alert_windup_s:
				...
				if debug_logs:
					print("[EliteSpawner] vision trigger (", dist, " m, looking=", player_looking,
						") → P+", p_bonus, "% M+", m_bonus, "% (scaled from ", monster_start_bonus_pct, "%)")
				_pause_any_non_idle()
				_start_battle(player, p_bonus, m_bonus)
				return

	# Idle frame: keep idle + rotate/reorient (rotation funcs are no-ops if disabled)
	_ensure_idle_playing()
	_update_idle_rotation(dt, player)
	_update_big_reorient(dt, player)
...
func _start_battle(player: Node3D, player_bonus_pct: int, monster_bonus_pct: int) -> void:
	...
	_armed = false
	if debug_logs:
		print("[EliteSpawner] request encounter id=", monster_id, " power=", power_level,
			" Pbonus=", player_bonus_pct, " Mbonus=", monster_bonus_pct)
...
# NOTE: FOV now uses the same yaw used to rotate the rig, so it turns with the mob.
func _is_in_my_fov(player: Node3D, my_pos: Vector3) -> bool:
	...
	if debug_vision:
		print("[EliteSpawner] FOV dot=%.3f cos_half=%.3f -> %s" % [dotv, cos_half, str(dotv >= cos_half)])
	return dotv >= cos_half

func _forward_from_yaw(yaw: float) -> Vector3:
	# Forward is -Z in Godot → (sin, 0, cos)
...
func _has_los(player: Node3D, my_pos: Vector3) -> bool:
	...
	if hit.is_empty():
		if debug_vision:
			print("[EliteSpawner] LoS: clear")
		return true

	if debug_vision:
		print("[EliteSpawner] LoS: blocked by %s" % [str(hit.get("collider","<unknown>"))])
...
func _resolve_idle_name() -> void:
	...
	for cand in idle_anim_candidates:
		if cand != "" and _ap.has_animation(cand):
			...
			if debug_anim:
				print("[EliteSpawner] Idle candidate chosen: ", _idle_name)
			return
	if debug_anim:
		print("[EliteSpawner] No idle candidate found; leaving animations off")

func _try_play_idle_now() -> void:
	if _ap == null:
		return
	# Kill any auto-play loop often named SenseLoop on rigs.
	if _ap.current_animation == "SenseLoop":
		if debug_anim: print("[EliteSpawner] Stopping SenseLoop autoplay")
		_ap.stop()
	if _ap.has_method("set"):
		_ap.set("autoplay", "")
	if _idle_name == &"":
		_resolve_idle_name()
	if _idle_name != &"" and _ap.has_animation(String(_idle_name)):
		...
		_last_anim = _idle_name
		if debug_anim:
			print("[EliteSpawner] Switch anim: <start> -> ", _idle_name)
	else:
		# No valid idle → keep playback stopped (static prop look).
		_ap.stop()
...
func _ensure_idle_playing() -> void:
	...
	if cur != String(_idle_name):
		if debug_anim:
			print("[EliteSpawner] Switch anim: ", cur, " -> ", _idle_name)
		_ap.play(String(_idle_name))
		_last_anim = _idle_name
	elif not _ap.playback_active:
		if debug_anim:
			print("[EliteSpawner] Playback inactive; reactivating Idle")
		_ap.play(String(_idle_name))
...
func _pause_any_non_idle() -> void:
	...
	if _ap == null:
		return
	if _idle_name != &"":
		if _ap.current_animation != String(_idle_name):
			if debug_anim:
				print("[EliteSpawner] Force Idle before battle: ", _idle_name)
		_ap.play(String(_idle_name))
	else:
		_ap.stop()
...
func _disable_root_tracks_for_idle() -> void:
	...
	for i: int in anim.get_track_count():
		...
		if is_root and is_transform_track:
			...
			disabled += 1
	if disabled > 0 and debug_anim:
		print("[EliteSpawner] Disabled ", disabled, " root POS/ROT/SCALE tracks on Idle (", rig.name, ")")
...
func _wrap_angle(a: float) -> float:
	var t: float = fmod(a, TAU)
	return t + TAU if t < 0.0 else t
...
func _on_body_entered(body: Node) -> void:
	...
	if p != null and (p.is_in_group("player") or p.is_in_group("player_controller")):
		_player_in_range = p
		if debug_logs:
			print("[EliteSpawner] player in range")

func _on_body_exited(body: Node) -> void:
	if _player_in_range == body:
		_player_in_range = null
		if debug_logs:
			print("[EliteSpawner] player left")
...
func _spawn_visual() -> Node3D:
	...
	if catalog == null:
		push_warning("[EliteSpawner] MonsterCatalog missing; creating fallback box.")
		return _make_fallback_box()
	var slug: StringName = monster_id
	var vis := catalog.instantiate_visual(self, slug)
	if vis == null:
		push_warning("[EliteSpawner] No visual for %s; using fallback." % String(slug))
...
# ---------------------------------------------------------------------------------
# API for EncounterDirector cleanup
# ---------------------------------------------------------------------------------
...
func mark_defeated_and_cleanup() -> void:
	...
	await tw.finished
	if debug_logs:
		print("[EliteSpawner] despawn after victory")
	queue_free()
...
func _is_world_gated() -> bool:
	if get_tree().paused:
		return true
	var dir := get_node_or_null(^"/root/EncounterDirector")
	if dir != null and dir.has_method("is_busy") and bool(dir.call("is_busy")):
		if debug_logs:
			print("[DIRECTOR] gate active; elite spawner idle (", _my_path_str, ")")
		return true
	return false

func _current_step_index() -> int:
...
func _is_under_cooldown_here() -> bool:
	var ar := get_node_or_null(^"/root/AnchorRegistry")
	if ar == null:
		return false
	if not ar.has_method("anchor_key_for_pos") or not ar.has_method("is_under_cooldown"):
		return false

	# Use the active save slot for floor resolution (refactor-consistent)
	var slot_i: int = SaveManager.active_slot()
	var floor_i: int = SaveManager.get_current_floor(slot_i)

	var key: String = String(ar.call("anchor_key_for_pos", floor_i, global_transform.origin))
	var step_now: int = _current_step_index()
	var active: bool = bool(ar.call("is_under_cooldown", key, step_now))

	if active and debug_logs:
		var left: int = int(ar.call("cooldown_left", key, step_now))
		print("[EliteSpawner] cooldown active (", left, " steps) at ", key)
