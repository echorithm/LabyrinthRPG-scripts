...
func _ready() -> void:
	...
	if use_anim_auto:
		...
		if debug_anim: print("[EliteSpawner] AnimAuto.play_idle -> ", str(_anim_auto_owns_idle))
...
func _forward_from_yaw(yaw: float) -> Vector3:
	# Matches how we reconstruct transforms in _set_yaw_rad
	# Forward is -Z in Godot, which corresponds to (sin, 0, cos) here.
