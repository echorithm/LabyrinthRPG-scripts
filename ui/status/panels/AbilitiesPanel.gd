...
func _ready() -> void:
	if _list and not _list.item_selected.is_connected(_on_item_selected):
		_list.item_selected.connect(_on_item_selected)
	# Prepare gesture line under preview (create if absent)
...
func refresh() -> void:
	...
	print("[AbilitiesPanel] refresh(slot=%d)" % slot_i)
	...
	print("[AbilitiesPanel] refresh(slot=%d) run keys=%s" % [slot_i, str(run.keys())])

	var st_any: Variant = run.get("skill_tracks")
	if typeof(st_any) != TYPE_DICTIONARY:
		print("[AbilitiesPanel] no/invalid 'skill_tracks' in run (type=%d)" % typeof(st_any))
		return
...
# --- helpers (add these near the bottom of the file, before _gesture_points_for_gesture) ---
...
func _gesture_points_for_gesture(gesture_id: String) -> PackedVector2Array:
	# Since you mapped everything to arc_slash, make sure it returns something non-empty
	match gesture_id:
		"arc_slash":
...
func _normalize_right_layout() -> void:
	...
	if details_box and _stats_lbl:
		...
		if _stats_scroll == null:
			...
			# Only set autowrap if it’s a Label; for RichTextLabel we rely on scrolling
			if _stats_lbl is Label:
	...
	if _gesture_preview:
		...
		if details_box:
			details_box.size_flags_stretch_ratio = 2.0
