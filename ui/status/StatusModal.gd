...
func on_opened() -> void:
	...
	_switch_to(_active_tab)  # keeps last-opened tab if you reopen
...
func _build_tabs_if_needed() -> void:
	if _tabs.get_child_count() == 0:
		...
		for i in labels.size():
			...
			b.text = labels[i]
			b.toggle_mode = true
			b.button_group = ButtonGroup.new() if i == 0 else (_tabs.get_child(0) as Button).button_group
...
func _switch_to(tab_i: int) -> void:
	...
	if _content.custom_minimum_size.y < 320.0:
		_content.custom_minimum_size.y = 320.0
...
func _resolve_slot() -> int:
	# If an explicit slot is provided, use it.
	if slot > 0:
		return slot
	# Otherwise pull from RunState if available.
