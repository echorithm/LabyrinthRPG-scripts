...
# New behavior
@export var appear_on_touch: bool = true
@export var trigger_side: int = TriggerSide.LEFT
@export var only_touchscreen: bool = true
@export var dynamic_center: bool = true   # default on when appearing at touch
...
func _process(_d: float) -> void:
	if _label:
		_label.text = "move " + str(_value).substr(0, 14)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if e.pressed and _pointer_id == -1 and _trigger_allows(e.position, rect) and (not respect_ui or not _ui_blocks(e.position)):
			if appear_on_touch:
				visible = true
				var local: Vector2 = e.position - rect.position
				_center = local
				_start_center = _center
				_knob_pos = Vector2.ZERO
			elif dynamic_center:
				var local2: Vector2 = e.position - rect.position
				_center = local2
				_start_center = _center
		...
		if event is InputEventMouseButton and (event as InputEventMouseButton).button_index == MOUSE_BUTTON_LEFT:
			...
			if mb.pressed and _pointer_id == -1 and _trigger_allows(mb.position, rect) and (not respect_ui or not _ui_blocks(mb.position)):
				if appear_on_touch:
					visible = true
					var localm: Vector2 = mb.position - rect.position
					_center = localm
					_start_center = _center
					_knob_pos = Vector2.ZERO
				elif dynamic_center:
					var localm2: Vector2 = mb.position - rect.position
					_center = localm2
					_start_center = _center
...
func _ui_blocks(global_pos: Vector2) -> bool:
	...
	# Align hover to the touch point so we can query the right control.
