...
const COL_RING_ON   := Color(0.4, 1.0, 0.6, 0.9)
...
func _begin(id: int, pos: Vector2, rect: Rect2) -> void:
	...
	_pointer_id = id
	if dynamic_center:
		var local: Vector2 = pos - rect.position
		_center = local
