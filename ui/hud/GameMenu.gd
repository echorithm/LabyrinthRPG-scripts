...
func _is_in_battle() -> bool:
	# Any live controller in the group means we're in an encounter.
...
func _open_status() -> void:
	...
	if rs:
		...
		elif rs.has_method("reload"): # fallback if you already have a reload()
			rs.call("reload")
		else:
			# ultimate fallback: load and then just emit changed so listeners repaint
			var _tmp := SaveManager.load_run(SaveManager.DEFAULT_SLOT)
