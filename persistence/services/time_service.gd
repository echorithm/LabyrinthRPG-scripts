...
const DEFAULT_SLOT: int = SaveManager.DEFAULT_SLOT
...
static func realtime_boot_apply(slot: int = DEFAULT_SLOT, max_backfill_days: int = 14) -> float:
...
static func realtime_heartbeat(slot: int = DEFAULT_SLOT, max_backfill_days: int = 14) -> float:
...
static func realtime_snapshot(slot: int = DEFAULT_SLOT) -> Dictionary:
...
static func on_step(is_in_battle: bool, slot: int = DEFAULT_SLOT) -> void:
...
static func commit_and_reset(slot: int = DEFAULT_SLOT) -> float:
	var run: Dictionary = SaveManager.load_run(slot)
	var a: float = float(_dgetf(run, "floor_time_accum_min", 0.0))
...
static func snapshot(slot: int = DEFAULT_SLOT) -> Dictionary:
	var run: Dictionary = SaveManager.load_run(slot)
	return {
