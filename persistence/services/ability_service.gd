...
static func xp_snapshot(ability_id: String, slot: int = 1) -> Dictionary:
	# { level, xp_current, xp_needed, cap_band, unlocked }
	if ability_id.is_empty():
		return {}
...
static func award_xp(ability_id: String, add_xp: int, slot: int = 1) -> Dictionary:
	# Applies XP to one skill track (META). Levels up within cap and returns the updated snapshot.
	if ability_id.is_empty():
		return {}
	...
	if not st.has(ability_id):
		# create locked track if missing (caller usually checks unlock)
