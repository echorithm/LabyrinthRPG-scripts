...
static func award_character_xp(amount: int, slot: int = SaveManager.DEFAULT_SLOT) -> Dictionary:
	var add: int = max(0, amount)
	...
	var xp_cur: int = int(sb.get("xp_current", 0))
	var xp_need: int = int(sb.get("xp_needed", int(round(90.0 * pow(1.13, float(lvl - 1))))))

	# Add XP and level up as needed (your curve)
	xp_cur += add
	while xp_cur >= xp_need:
		xp_cur -= xp_need
		lvl += 1
		xp_need = int(round(90.0 * pow(1.13, float(lvl - 1))))

	# Persist back into META
	sb["level"] = lvl
	sb["xp_current"] = xp_cur
	sb["xp_needed"] = xp_need
	pl["stat_block"] = sb
	gs["player"] = pl
	SaveManager.save_game(gs, slot)

	# Return a snapshot (caller expects level at least)
	return { "level": lvl, "xp_current": xp_cur, "xp_needed": xp_need }
