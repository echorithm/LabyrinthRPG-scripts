...
static func defaults(_meta_schema_version: int) -> Dictionary:
	...
	return {
		...
		# Per-session
		"skill_xp_delta": {},        # legacy: ad-hoc awards (kept)
		"ability_use_counts": {},    # per-run diminishing
		"ability_xp_pending": {},    # encounter_id -> { ability_id: xp }
		"action_skills": {},         # optional per-action table (kept)
	}
...
static func migrate(d_in: Dictionary, _meta_schema_version: int) -> Dictionary:
	...
	# equipment & tags
	var eq: Dictionary = _S.to_dict(d.get("equipment", {}))
	var eq_norm: Dictionary = {
		"head": null, "chest": null, "legs": null, "boots": null,
		"mainhand": null, "offhand": null, "ring1": null, "ring2": null, "amulet": null
	}
	for k in eq_norm.keys():
		eq_norm[k] = eq.get(k, null)
	d["equipment"] = eq_norm
	...
	# ensure each row normalizes ints
