...
@export var look_stick: NodePath                     # e.g. ../../../HUD/VirtualStickRight

var yaw: float = 0.0
var pitch: float = 0.0

@onready var pivot: Node3D = $".."
@onready var _stick: VirtualStick = get_node_or_null(look_stick) as VirtualStick
...
func _ready() -> void:
	...
	if _stick == null:
		_stick = get_tree().get_first_node_in_group("look_stick") as VirtualStick
	if _stick == null:
		push_warning("CameraGyro: Look Stick not assigned or found (set 'look_stick' NodePath or group).")
...
func _process(delta: float) -> void:
	...
	if _stick == null and look_stick != NodePath(""):
		_stick = get_node_or_null(look_stick) as VirtualStick

	# ===== Right stick (independent invert) =====
	var used_stick := false
	if _stick != null:
	...
	var yaw_dir_keys: float
	if invert_yaw:
		yaw_dir_keys = 1.0
	else:
		yaw_dir_keys = -1.0
	var pitch_dir_keys: float
	if invert_pitch:
		pitch_dir_keys = 1.0
	else:
		pitch_dir_keys = -1.0
	...
	if gyro_enabled and not used_stick:
		...
		if g != Vector3.ZERO:
			var yaw_dir_gyro: float
			if invert_yaw:
				yaw_dir_gyro = 1.0
			else:
				yaw_dir_gyro = -1.0
			var pitch_dir_gyro: float
			if invert_pitch:
				pitch_dir_gyro = 1.0
			else:
				pitch_dir_gyro = -1.0
